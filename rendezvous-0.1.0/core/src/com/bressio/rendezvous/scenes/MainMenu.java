...
public class MainMenu implements Screen {
    ...
    private boolean isMusicPlaying = false;  // Make sure this is initialized.
    ...
    @Override
public void show() {
    Sound testSound = Gdx.audio.newSound(Gdx.files.internal("audio/music/kordhell_killers.mp3"));
    testSound.play();

    // Load the music (only if it hasn't been loaded before)
    if (backgroundMusic == null) {
        System.out.println("Loading music...");
        backgroundMusic = Gdx.audio.newMusic(Gdx.files.internal("audio/music/kordhell_killers.mp3"));
        backgroundMusic.setLooping(true);  // Make it loop continuously
        backgroundMusic.setVolume(1.0F);   // Set the volume (0 to 1)
    }
    ...
    if (!isMusicPlaying) {
        ...
        System.out.println("Music playing status: " + backgroundMusic.isPlaying());
        isMusicPlaying = true;
    }
}
...
public void hide() {
    if (backgroundMusic != null) {
        backgroundMusic.pause(); //  Use pause() to resume later, stop() to restart.
